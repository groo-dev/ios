...
- **The `selectedTab` UserDefaults key is shared** between `phoneSelection` and `padSelection`. Only one of the two is ever bound to a live `TabView` in a given process, so they cannot fight — but do not add a third writer.
