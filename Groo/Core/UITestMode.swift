...
/// In-memory Pass "server" for the per-item record endpoints: key-info, the
/// delta sync, and single-record create/update/delete with the real per-record
/// optimistic-locking contract (409 on a stale expectedVersion).
///
/// Seeded with an empty vault — no records — and the wrapped vault key the
/// app's own CryptoService produces, so PassService's unlock (derive → unwrap →
/// sync) has something real to unwrap.
final class UITestVaultServer: @unchecked Sendable {
    static let shared = UITestVaultServer()

    private let lock = NSLock()
    private var wrappedVaultKey: String // base64 — vault key wrapped by the passphrase-derived key
    private var wrapIv: String          // base64
    ...
    private init() {
        ...
        do {
            // Stands in for the web app: the real vault key is generated once,
            // on vault creation, and never regenerated by iOS. Mirrors that here.
            ...
            let wrapped = try crypto.wrapKey(vaultKey, using: wrappingKey)
            self.wrappedVaultKey = wrapped.ciphertext
            self.wrapIv = wrapped.iv
        } catch {
    ...
    func response(for request: URLRequest) -> (status: Int, body: Data) {
        ...
        if method == "GET", path == "/v1/vault/key-info" {
        ...
        // No sharing keypair in the UI-test fixture. 404 is a normal state for
        // an account that has never shared, and the app treats it as one.
        if path == "/v1/vault/private-key" {
            return (404, Data(#"{"error":"PRIVATE_KEY_NOT_SET"}"#.utf8))
        }

        if method == "GET", path == "/v1/vault/records" {
            let since = Int(query?.first { $0.name == "since" }?.value ?? "0") ?? 0
            let page = records.values
                .filter { $0.seq > since }
                .sorted { $0.seq < $1.seq }
            return (200, encode(SharedRecordsResponse(
                records: page,
                nextSeq: page.last?.seq ?? since,
                hasMore: false
            )))
        }

        if method == "POST", path == "/v1/vault/records" {
            ...
            guard records[write.id] == nil else {
                return (409, Data(#"{"error":"RECORD_EXISTS","code":"RECORD_EXISTS"}"#.utf8))
            }
            let stored = store(write, version: 1, createdAt: nil)
            return (200, encode(SharedRecordWriteResponse(
                id: stored.id, seq: stored.seq, version: stored.version
            )))
        }

        if path.hasPrefix("/v1/vault/records/") {
            let id = String(path.dropFirst("/v1/vault/records/".count))
            ...
            if method == "PUT" {
                ...
                guard let existing = records[id] else {
                    return (404, Data(#"{"error":"RECORD_NOT_FOUND","code":"RECORD_NOT_FOUND"}"#.utf8))
                }
                guard write.expectedVersion == existing.version else {
                    return (409, Data(#"{"error":"RECORD_CONFLICT","code":"RECORD_CONFLICT"}"#.utf8))
                }
                let stored = store(write, version: existing.version + 1, createdAt: existing.createdAt)
                return (200, encode(SharedRecordWriteResponse(
                    id: stored.id, seq: stored.seq, version: stored.version
                )))
            }
            ...
            if method == "DELETE" {
                ...
                return (200, encode(SharedRecordDeleteResponse(id: id, seq: changeSeq)))
            }
        }
    ...
    private func store(
        _ write: SharedRecordWriteRequest,
        version: Int,
        createdAt: Int?
    ) -> SharedServerRecord {
        changeSeq += 1
        ...
        let stored = SharedServerRecord(
            id: write.id,
            encryptedData: write.encryptedData,
            iv: write.iv,
            wrappedRecordKey: write.wrappedRecordKey,
            wrapIv: write.wrapIv,
            version: version,
            seq: changeSeq,
            isDeleted: false,
